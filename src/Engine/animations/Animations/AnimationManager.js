...
class AnimationManager {
  ...
  load(item) {
    if (!item.id || !item.film)
      throw Error("given animation doesn't have id/film");
    ...
    this._animations[item.id] = {
      animation: new FrameRangeAnimation(item),
      film: film,
    };
  }
  ...
  register(anim, film) {
    // class Animation
    this._animations[anim.id] = {
      animation: anim,
      film: animationFilmHolder.getFilm(film),
    };
  }
