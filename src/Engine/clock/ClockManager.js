...
class Callback {
  ...
  constructor({ cb, args, delay = 1000, repeat = false }) {
    if (!cb && typeof cb !== "function")
      throw Error("Created callback without a valid function");
    this._id = idCreator.randomString({
      capital: true,
      small: true,
      number: true,
      length: 10,
...
export default class ClockManager extends Manager {
  ...
  cancelCallBack(cbObject) {
    if (this._callbacks[cbObject.id]) delete this._callbacks[cbObject.id];
  }
}
