...
export default class OptionManager {
  ...
  setOptionCode(opt, code) {
    if (this._regOptions[opt]) this._regOptions[opt].onChange = code;
  }

  getOptionCode(opt) {
    const option = this._regOptions[opt];
    if (option) return option.onChange;
    return { text: "", code: "" };
  }

  removeOption(opt) {
    if (!this._regOptions[opt])
      throw Error("Tried to remove an option that doesn't exist");
    delete this._regOptions[opt];
  }

  getOptionTag(opt) {
    if (!this._regOptions[opt])
      throw Error("Tried to get an option that doesn't exist");
