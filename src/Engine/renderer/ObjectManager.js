...
class ObjectManager extends Manager {
  ...
  removeConstructor(name) {
    if (this._constructors[name]) delete this._constructors[name];
  }
  ...
  addSystemObject(objID) {
    ...
    if (index !== -1)
      throw Error("trying to add a system object that is already registered");
...
clickWrapper.style.top =
  (window.innerHeight - envObj._windowHeight / envObj._aspectRatio) / 2 + "px";
