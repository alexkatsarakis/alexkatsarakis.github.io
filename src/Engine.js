...
class _Engine {
  ...
  installManager(name, manager) {
    assert.check(
      !this._managers[name],
      "Installing a manager that already exists"
    );
...
game.input = () => {
  ...
  Engine.InputManager.getReleasedKeys().forEach((key) =>
    utils.inputHandler("Unpressed" + key)
  );
  Engine.InputManager.getPressedKeys().forEach((key) =>
    utils.inputHandler("Pressed" + key)
  );
};
