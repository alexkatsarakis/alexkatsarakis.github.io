...
class CommunicationManager {
  ...
  createTable(name, structure) {
    ...
    requestToServer("POST", serverPrefix + "create/" + name, structure).then(
      (res) => console.log(res)
    );
  }
  ...
  addItemToTable(name, value) {
    ...
    requestToServer("POST", serverPrefix + "addItem/" + name, value).then(
      (res) => console.log(res)
    );
  }
  ...
  updateItemToTable(name, ident, value) {
    requestToServer(
      "POST",
      serverPrefix + "updateItem/" + name,
      JSON.stringify({ value: value, ident: ident })
