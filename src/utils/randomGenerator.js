...
class Randomizer {
  ...
  randomString({
    ...
    assert.check(
      fromStringToChoose.length > 0,
      "you didn't choose any char set"
    );
    let accum = "";
    for (let i = 0; i < length; i++) {
      accum +=
        fromStringToChoose[
          Math.floor(Math.random() * fromStringToChoose.length)
        ];
    }
    return accum;
  }
  ...
  randomHex({ length = 6 }) {
    ...
    for (let i = 0; i < length; i++) {
      accum +=
        this._data["hex"][Math.floor(Math.random() * this._data["hex"].length)];
    }
    return accum;
  }
