...
function actionsDropdown() {
  ...
  function createDropdown() {
    ...
    dropdown.style.left =
      document.getElementById("toolbar_actions-icon").getBoundingClientRect()
        .x + "px";
...
function objectsDropdown() {
  ...
  function createDropdown() {
    ...
    dropdown.style.left =
      document
        .getElementById("toolbar_liveobjects-icon")
...
function objCreationDropdown() {
  ...
  function createDropdown() {
    ...
    dropdown.style.left =
      document
        .getElementById("toolbar_objCreation-icon")
        .getBoundingClientRect().x + "px";
    document.body.appendChild(dropdown);
...
function openSettings() {
  bb.fastGet(
    "UI",
    "installUI"
  )({ name: setWindow.name, link: setWindow.link, cb: setWindow.cb });
...
function openInventory() {
  bb.fastGet(
    "UI",
    "installUI"
  )({ name: invWindow.name, link: invWindow.link, cb: invWindow.cb });
...
function onSearch(ev) {
  ...
  for (let i in objects) {
    ...
    for (let j in states) {
      if (j.toLowerCase().includes(query.toLowerCase()))
        tempArray.push(obj.name + "#" + j);
    }
  ...
  arr.forEach((name) => {
    ...
    item.insertAdjacentHTML(
      ...
      name.replace(
        query,
        '<b style="color:var(--secondary-color);">' + escape(query) + "</b>"
      )
    );
...
function onToolbarLoaded() {
  ...
  backBut.onclick = () => {
    if (bb.fastGet("settings", "Show Prompt On Actions")) {
      bb.fastSet("events", "openPrompt", {
        ...
        description: `${tr.get("If you accept")} ${tr.get(
          "you will be redirected"
        )} ${tr.get("and")} ${tr.get("you will lose")} ${tr.get(
          "all the progress"
        )} ${tr.get("if you haven't saved")}`,
        onAccept: () => {
          window.location = "/";
        },
      });
  ...
  document.getElementById("toolbar_saveLocal").title =
    'Save the current game and go to game "local" to continue';
  ...
  document.getElementById("toolbar_downloadState").title =
    "Download the current game state for later use";
  document.getElementById("toolbar_downloadState-icon").style.cursor =
    "pointer";
