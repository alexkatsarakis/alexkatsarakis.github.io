...
function addStateButtonHandler() {
  ...
  accept.onclick = () => {
    ...
    bb.fastSet(
      "events",
      "showFeedback",
      `${textValue} state was added to ${focusedObj.name}`
    );
  };
}
...
function createStatesView(states, currentState) {
  ...
  document.getElementById("mainInfoBox-body-states-title-text").innerHTML =
    tr.get("States");
  ...
  for (let i in states) {
    ...
    const gotoState = document.getElementById(
      `mainInfoBox-body-states-main-edit-${i}`
    );
    if (i === currentState) {
      gotoState.remove();
      ...
      gotoState.onclick = () => {
        ...
        bb.fastSet(
          "events",
          "showFeedback",
          `${focusedObj.name} transitioned to ${i} state`
        );
      };
    }
  }
...
function addAttributeButtonHandler() {
  ...
  accept.onclick = () => {
    ...
    bb.fastSet(
      "events",
      "showFeedback",
      `${textValue} attribute was added to ${focusedObj.name}`
    );
  };
}

function createAttributesView(attrs) {
  const attributesInfo = document.getElementById(
    "mainInfoBox-body-attributes-main"
  );
  attributesInfo.innerHTML = "";

  document.getElementById("mainInfoBox-body-attributes-title-text").innerHTML =
    tr.get("Attributes");
  ...
  for (let i in attrs) {
    ...
    const rmBut = document.getElementById(
      `mainInfoBox-body-attributes-main-remove-${i}`
    );
    if (focusedObj.getValueTag(i) === "user") {
      ...
      rmBut.onclick = () => {
        ...
        bb.fastSet(
          "events",
          "showFeedback",
          `Removed attribute ${i} from ${focusedObj.name}`
        );
      };
    } else {
    ...
    const editBut = document.getElementById(
      `mainInfoBox-body-attributes-main-edit-${i}`
    );
    editBut.onclick = () => {
      ...
      inp.onchange = (ev) => {
        ...
        bb.fastSet(
          "events",
          "showFeedback",
          `${focusedObj.name} attribute ${i} changed to ${val}`
        );
        if (isNaN(val) || val === "") focusedObj.setValue(i, val);
        else focusedObj.setValue(i, Number.parseFloat(val));
      };
...
function addEventsButtonHandler() {
  ...
  accept.onclick = () => {
    ...
    bb.fastSet(
      "events",
      "showFeedback",
      `${textValue} event was added to ${focusedObj.name}`
    );
  };
}
...
function createEventsView(events) {
  ...
  document.getElementById("mainInfoBox-body-events-title-text").innerHTML =
    tr.get("Events");
  ...
  for (let i in events) {
    ...
    if (focusedObj.getEventTag(i) === "user") {
      const rmBut = document.getElementById(
        `mainInfoBox-body-events-main-remove-${i}`
      );
      rmBut.style.fill = "red";
      rmBut.style.cursor = "pointer";
      rmBut.onclick = () => {
        ...
        bb.fastSet(
          "events",
          "showFeedback",
          `Removed Event ${i} from ${focusedObj.name}`
        );
      };
    }
  }
...
function addFlagsButtonHandler() {
  ...
  accept.onclick = () => {
    ...
    bb.fastSet(
      "events",
      "showFeedback",
      `${textValue} flag was added to ${focusedObj.name}`
    );
  };
}
...
function createFlagsView(flags) {
  ...
  document.getElementById("mainInfoBox-body-flags-title-text").innerHTML =
    tr.get("Flags");
  ...
  for (let i in flags) {
    ...
    if (obj.getOptionTag(i) === "user") {
      const rmBut = document.getElementById(
        `mainInfoBox-body-flags-main-remove-${i}`
      );
      rmBut.style.fill = "red";
      rmBut.style.cursor = "pointer";
      rmBut.onclick = () => {
        ...
        bb.fastSet(
          "events",
          "showFeedback",
          `Removed flag ${i} from ${obj.name}`
        );
      };
    }

    const editBut = document.getElementById(
      `mainInfoBox-body-flags-main-edit-${i}`
    );
    editBut.onclick = () => {
      editBut.style.display = "none";
      text.innerHTML = i + ": ";
      ...
      opt.onchange = (ev) => {
        ...
        bb.fastSet(
          "events",
          "showFeedback",
          `${obj.name} flag ${i} changed to ${ev.target.value}`
        );
      };
...
function addCollisionsButtonHandler() {
  ...
  accept.onclick = () => {
    ...
    bb.fastSet(
      "events",
      "showFeedback",
      `${textValue} collision was added to ${focusedObj.name}`
    );
  };
}
...
function createCollisionsView(cols) {
  ...
  document.getElementById("mainInfoBox-body-collisions-title-text").innerHTML =
    tr.get("Collisions");
  ...
  for (let name in cols) {
    ...
    const rmBut = document.getElementById(
      `mainInfoBox-body-collisions-main-remove-${name}`
    );
    rmBut.style.fill = "red";
    rmBut.style.cursor = "pointer";
    rmBut.onclick = () => {
      ...
      bb.fastSet(
        "events",
        "showFeedback",
        `Removed collision ${name} from ${obj.name}`
      );
    };
  }
}
...
function fillObjectActions(obj) {
  ...
  if (
    Engine.hasManager("ClipboardManager") &&
    !Engine.ObjectManager.isSystemObject(obj.id)
  ) {
    wrapper.insertAdjacentHTML(
      "beforeend",
    ...
    removeBut.onclick = () => {
      if (bb.fastGet("settings", "Show Prompt On Actions")) {
        bb.fastSet("events", "openPrompt", {
          ...
          description: `${tr.get("If you accept")} ${tr.get("object")} ${
            obj.name
          } ${tr.get("will get removed")}`,
          onAccept: () => {
            obj.remove();
            bb.fastSet("state", "focusedObject", undefined);
...
function updateInfo(obj) {
  ...
  document.getElementById("mainInfoBox-head-categ").innerHTML = `${tr.get(
    "Category"
  )}: ${tr.get(obj.getCategory())}`;
