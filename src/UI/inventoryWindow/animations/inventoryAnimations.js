...
const FRAnimator =
  Engine.AnimationManager.getAnimatorCategory("FrameRangeAnimator");
const FRAnimation = Engine.AnimationManager.getAnimationCategory(
  "FrameRangeAnimation"
);
...
function createPopUp(
  ...
  for (let i = 0; i < film.totalFrames; ++i) {
    ...
    previewCtx.drawImage(
      bb.fastGet("assets", film.bitmap),
      box.x,
      box.y,
      box.width,
      box.height,
      (preview.width - endW) / 2,
      0,
      endW,
      width
    );
  ...
  delaySliderPrompt.classList +=
    "animationWorkshopCreate_popup_editarea_prompt";
  ...
  let currPos = {
    x:
      mainAreaCanvas.width / 2 -
      ((firstBox.width / firstBox.height) * (mainAreaCanvas.height / 3)) / 2,
  ...
  animator.onAction = (th) => {
    ...
    if (currPos.x > mainAreaCanvas.width) currPos.x = -firstBox.width;
    else if (currPos.x + firstBox.width < 0)
      currPos.x = mainAreaCanvas.width - firstBox.width;
    if (currPos.y > mainAreaCanvas.height) currPos.y = -firstBox.height;
  ...
  function saveAnimation() {
    if (idInput.value !== "") {
      Engine.AnimationManager.registerNewAnimation(
        new FRAnimation({
          id: idInput.value,
          start: 0,
          end: film.totalFrames - 1,
  ...
  delaySlider.addEventListener("change", () => {
    delaySliderPrompt.innerHTML = `${tr.get("Delay")}: ${escape(
      delaySlider.value
    )}`;
    restartAnimation();
  });
