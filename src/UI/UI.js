...
const notDefaultUIs = ["dummyGameUI"];
...
class UIManager {
  ...
  installUI({ name, link, cb, removable, loadOnInstall }) {
    if (this._UIInstalled[name])
      throw Error("Installing an already installed UI");
  ...
  removeUI(name) {
    if (!this._UIInstalled[name])
      throw Error("Removing a UI that doesn't exist");
  ...
  loadUI(name) {
    ...
    if (!this._UIInstalled[name] || index !== -1)
      throw Error("Trying to load a UI that isn't installed");
  ...
  hideUI(name) {
    const index = this._UILoaded.findIndex((item) => item === name);
    if (!this._UIInstalled[name] || index === -1)
      throw Error("Trying to hide a UI that isn't installed");
...
bb.fastInstall("UI", "getUIs", () => {
  return uiManager.getUIs();
});
