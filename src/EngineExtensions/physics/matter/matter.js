...
export default class MatterJS {
    ...
    addToWorld(item){
        ...
        const objMapItem = this.objMap[item.getName()];
        for(let i in pos){
        ...
        if(pos.r !== undefined){
            const x = objMapItem.x + objMapItem.r/2;
            const y = objMapItem.y + objMapItem.r/2;
            objMapItem["phObject"] = Matter.Bodies.circle(x, y, objMapItem.r, options);
        }else{
            const x = objMapItem.x+ objMapItem.width/2;
            const y = objMapItem.y+ objMapItem.height/2;
    ...
    applyForce(object,position,force){
        const body = this.objMap[object.getName()].phObject;
        const pos = Matter.Vector.create(position[0],position[1]);
        const f = Matter.Vector.create(force[0],force[1]);
    ...
    update(){
        ...
        this._pe.world.bodies.forEach((body)=>{
            const realObj = bb.fastGet('Engine','ObjectManager').getObject(body.id);
            if(!realObj){
                this.removeFromWorld(body.name);
                return;
            }
            const proxy = this.objMap[body.name];
            const phObj = proxy.phObject;
